...
def resolve_char_uuid(client, target_uuid_str):
    target_norm = target_uuid_str.replace("-", "").lower()
    try:
        b = bytes.fromhex(target_norm)
        target_reversed = b[::-1].hex()
    except Exception:
        target_reversed = ""
        
    for service in client.services:
        for char in service.characteristics:
            char_norm = char.uuid.replace("-", "").lower()
            if char_norm == target_norm:
                return char.uuid
            if target_reversed and char_norm == target_reversed:
                print(f"  [안내] UUID 바이트 순서 뒤집힘 감지됨: {target_uuid_str} -> {char.uuid}")
                return char.uuid
    return target_uuid_str
...
async def run_update(address, do_app, do_storage):
    ...
    try:
        async with BleakClient(address, timeout=20.0) as client:
            ...
            global UUID_CTRL, UUID_DATA
            UUID_CTRL = resolve_char_uuid(client, UUID_CTRL)
            UUID_DATA = resolve_char_uuid(client, UUID_DATA)
